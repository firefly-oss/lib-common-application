...
import com.firefly.common.application.resolver.DefaultContextResolver;
...
/**
 * Integration test for FireflySessionManager with DefaultContextResolver.
 * 
 * <p>Tests the complete flow of:</p>
 * <ol>
 *   <li>Extracting partyId from X-Party-Id header</li>
 *   <li>Calling FireflySessionManager to get session</li>
 *   <li>Extracting roles and permissions from session</li>
 *   <li>Building AppContext with resolved data</li>
 * </ol>
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("FireflySessionManager Integration Tests")
class FireflySessionManagerIntegrationTest {
    ...
    @BeforeEach
    void setUp() {
        // Create resolver with mocked FireflySessionManager
    ...
    @Test
    @DisplayName("Should resolve roles from FireflySessionManager for party-level context")
    void shouldResolveRolesForPartyLevel() {
        // Given: Session with multiple contracts
        SessionContextDTO session = createSessionWithMultipleContracts();
    ...
    @Test
    @DisplayName("Should resolve roles from FireflySessionManager for contract-level context")
    ...
    @Test
    @DisplayName("Should resolve roles from FireflySessionManager for product-level context")
    ...
    @Test
    @DisplayName("Should resolve permissions from FireflySessionManager")
    void shouldResolvePermissionsFromSessionManager() {
        // Given: Session with role scopes (permissions)
        SessionContextDTO session = createSessionWithPermissions();
        when(sessionManager.createOrGetSession(any(ServerWebExchange.class)))
            .thenReturn(Mono.just(session));
    ...
    @Test
    @DisplayName("Should handle gracefully when FireflySessionManager is unavailable")
    ...
    @Test
    @DisplayName("Should handle errors from FireflySessionManager gracefully")
    ...
    void shouldExtractMultiplePermissionsFromMultipleScopes() {
        ...
        SessionContextDTO session = createSessionWithMultipleScopes();
        when(sessionManager.createOrGetSession(any(ServerWebExchange.class)))
            .thenReturn(Mono.just(session));
    ...
    void shouldFilterInactiveContracts() {
        ...
        SessionContextDTO session = createSessionWithInactiveContracts();
        when(sessionManager.createOrGetSession(any(ServerWebExchange.class)))
            .thenReturn(Mono.just(session));
    ...
    void shouldFilterInactiveRoleScopes() {
        ...
        SessionContextDTO session = createSessionWithInactiveScopes();
        when(sessionManager.createOrGetSession(any(ServerWebExchange.class)))
            .thenReturn(Mono.just(session));
    ...
    private SessionContextDTO createSessionWithMultipleContracts() {
    ...
    private SessionContextDTO createSessionWithPermissions() {
    ...
    private SessionContextDTO createSessionWithMultipleScopes() {
    ...
    private SessionContextDTO createSessionWithInactiveContracts() {
        return SessionContextDTO.builder()
            .sessionId(UUID.randomUUID().toString())
            .partyId(testPartyId)
            .activeContracts(List.of(
    ...
    private SessionContextDTO createSessionWithInactiveScopes() {
        List<RoleScopeInfoDTO> scopes = List.of(
